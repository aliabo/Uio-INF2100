...
public class Scanner {
	...
	/**
	 * What char we hve at this position?
	 * <p>
	...
	/**
	 * TODO
	 * comment and rearrange this method , make it simpler
	 */
	/* A method to select the part of line that contains a token */
	private String selectTokenText(String s, int lineLength){
		while(!containsToken(s) && sourcePos < lineLength && charAtPositionIs() != ' '){
			// first of, remove comments and update linelength
			s = removeComments(s,lineLength);
			lineLength = sourceLine.length();
			// seeking for the right tokens
			if(charAtPositionIs() != ' ' && charAtPositionIs() != '\n' ){
				System.out.println(s);
				// reading 'c'
				if(charAtPositionIs() == '\''){
					try{
						//s don't contain something before
						if(s.equals("")) {
							if (sourcePos + 4 < lineLength)
								...
								if (sourceLine.substring(sourcePos, sourcePos + 4).equals("''''")) {
									...
									sourcePos += 4;
									System.out.println("return if '''' " + s);
									return s;
								}
							s += sourceLine.substring(sourcePos, sourcePos + 3);
							System.out.println("add togheter + 3 " + s);
							sourcePos += 3;
						}
						return s;
					}catch(Exception e){scannerError("Illegal char literal!");}
				}
				s += Character.toLowerCase(charAtPositionIs());
				sourcePos++;
			}
		}
		System.out.println("End of selectTokenText " + s);
		return s;
	}
	 ...
	 * if line is empty {@link #testIfEmpty}
	 *
	 *
	 * @param 	s			the location of the token, relative to the s argument
	...
	private int readNewLine(int lineLength, int nrOfCharToRemove) {
		if (sourcePos == lineLength) {
			readNextLine();
			lineLength = sourceLine.length();
			while (sourceLine.length() < nrOfCharToRemove && !sourceLine.equals("")) {
				readNextLine();
				lineLength = sourceLine.length();
			}
			sourcePos++;
